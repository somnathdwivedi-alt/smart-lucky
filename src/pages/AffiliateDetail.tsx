...
function EarningsCalculator({ visitors: defVis, conversion: defConv, aov: defAov }: { visitors: number; conversion: number; aov: number }) {
  ...
  const Slider = ({
    ...
    <div>
      <div className="mb-1.5 flex items-center justify-between">
        ...
        <span className="text-[14px] font-bold text-[#111827]">{fmt(value)}</span>
      </div>
      <input
  ...
  return (
    <div className="rounded-2xl border border-[#EEF2F7] bg-white p-6 shadow-[0_8px_24px_rgba(0,0,0,0.05)]">
      <p className="text-[20px] font-bold text-[#111827]">Earnings Calculator</p>
      ...
      <div className="mx-auto max-w-[1280px] px-6 pt-6 sm:px-8">
        ...
        <Reveal>
          <div className="grid gap-8 lg:grid-cols-[1fr_360px]">
            ...
            <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
              ...
              <div className="flex-1">
                ...
                <div className="mt-4 flex flex-wrap items-center gap-3">
                  ...
                  <span className="text-[14px] text-[#6B7280]">({aff.reviewCount})</span>
                </div>
                <p className="mt-4 max-w-[550px] text-[15px] leading-relaxed text-[#6B7280]">
                  {aff.description}
                </p>
            ...
            <div className="rounded-[20px] border border-[#EEF2F7] bg-white p-7 shadow-[0_8px_24px_rgba(0,0,0,0.05)]">
              <div className="divide-y divide-[#E5E7EB]">
                {aff.stats.map((s) => (
                  <div key={s.label} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                    <span className="text-[13px] font-medium text-[#6B7280]">{s.label}</span>
        ...
        {/* ─────── STICKY TABS ─────── */}
        <div className="sticky top-16 z-30 -mx-6 mt-14 border-b border-[#E5E7EB] bg-[#F8FAFC] px-6 sm:mx-0 sm:px-0">
        ...
        <section id="overview" ref={(el) => { sectionRefs.current.overview = el; }} className="scroll-mt-28">
          <Reveal>
            <div className="mt-14">
              ...
              <p className="mt-3 max-w-[700px] text-[15px] leading-relaxed text-[#6B7280]">
                {aff.description}
              </p>
              <div className="mt-6">
                <h3 className="text-[18px] font-bold text-[#111827]">Requirements</h3>
        ...
        <section id="commission" ref={(el) => { sectionRefs.current.commission = el; }} className="scroll-mt-28">
          <Reveal>
            <div className="mt-14">
              <h2 className="text-[24px] font-bold text-[#111827]">Commission Structure</h2>
              <div className="mt-6 grid items-start gap-6 lg:grid-cols-[1fr_380px]">
                {/* Table */}
                <div className="overflow-hidden rounded-[20px] border border-[#EEF2F7] bg-white shadow-[0_8px_24px_rgba(0,0,0,0.05)]">
                  <table className="w-full text-left text-[14px]">
                    <thead>
                      <tr className="bg-[#F8FAFC] text-[12px] font-semibold uppercase tracking-wide text-[#6B7280]">
        ...
        <section id="resources" ref={(el) => { sectionRefs.current.resources = el; }} className="scroll-mt-28">
          <Reveal>
            <div className="mt-14">
              <h2 className="text-[24px] font-bold text-[#111827]">Marketing Resources</h2>
              <div className="mt-6 grid grid-cols-2 gap-5 sm:grid-cols-3 md:grid-cols-5">
                {aff.resources.map((r) => {
                  const Icon = RESOURCE_ICONS[r.iconName] || ImageIcon;
                  return (
                    <button key={r.label} className="rounded-[20px] border border-[#EEF2F7] bg-white p-6 shadow-[0_8px_24px_rgba(0,0,0,0.05)] transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
        ...
        <section id="payout" ref={(el) => { sectionRefs.current.payout = el; }} className="scroll-mt-28">
          <Reveal>
            <div className="mt-14">
              <h2 className="text-[24px] font-bold text-[#111827]">Payout Methods</h2>
              <div className="mt-6 grid grid-cols-2 gap-5 md:grid-cols-4">
                {aff.payouts.map((p) => {
                  const Icon = PAYOUT_ICONS[p.iconName] || Banknote;
                  return (
                    <div key={p.label} className="rounded-[20px] border border-[#EEF2F7] bg-white p-6 shadow-[0_8px_24px_rgba(0,0,0,0.05)] transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
        ...
        <Reveal>
          <div className="mt-14">
            <h2 className="text-[24px] font-bold text-[#111827]">Similar Affiliate Programs</h2>
            <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
              {aff.related.map((r) => (
                <button
                  key={r.name}
                  onClick={() => navigate("affiliate", r.brandKey)}
                  className="rounded-[20px] border border-[#EEF2F7] bg-white p-6 shadow-[0_8px_24px_rgba(0,0,0,0.05)] text-left transition-all duration-300 hover:-translate-y-1 hover:shadow-lg"
                >
                  <div className="flex items-center gap-4">
                    <LogoBadge brand={getBrand(r.brandKey)} size={48} />
                    <div className="min-w-0 flex-1">
                      <p className="text-[15px] font-bold text-[#111827]">{r.name}</p>
